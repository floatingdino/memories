...
const MemoryType = async () => {
  ...
  return <Field type="select" label="What sort of memory is it?" options={[{value: "", label: "Choose one"}, ...memoryTypes.data?.map(({id, name}) => ({value: id, label: name}))!]} />;
}
...
export default function SubmitMemory() {
  ...
  async function action(formData: FormData) {
    ...
    if ((file as File).size > 0) {
      const {data} = await supabase.storage.from("memories").upload(`memories/${generateUID()}${(file as File).name}`, file as File, {cacheControl: "38400"})
      payload.file = data?.fullPath || ""
    }

    const resp = await supabase.from("memories").insert(payload)
